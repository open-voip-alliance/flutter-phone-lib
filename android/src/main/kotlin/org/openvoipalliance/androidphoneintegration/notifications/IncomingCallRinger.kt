...
class IncomingCallRinger(
    ...
    /**
     * If a bluetooth device is already connected when we start ringing, pin the ringtone to it straight away.
     */
    private fun MediaPlayer.applyPreferredOutputDevice() {
        val bluetoothDevice = manager.getDevices(AudioManager.GET_DEVICES_OUTPUTS).firstOrNull {
            it.type == AudioDeviceInfo.TYPE_BLUETOOTH_SCO || it.type == AudioDeviceInfo.TYPE_BLUETOOTH_A2DP
        } ?: return

        log("Bluetooth device already connected, routing ringtone directly to it: ${bluetoothDevice.productName}")
        setPreferredDevice(bluetoothDevice)
    }
